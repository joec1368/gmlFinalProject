...
def meta_test_adam(meta_test_task_id_list,
    ...
    for task_1, task_2, support, query, _ in load_tasks('test', task_pairs, dataname, K_shot, seed):
        ...
        acc_f1_over_batches(query_loader, test_model.PG, gnn, test_model.answering, 2, 'multi_class_classification',device=device)
...
def model_components(input_dim_F = 768, hid_dim_F = 768,token_num_F =20,input_dim_G=768,hidden_dim_G=768,out_dim_G=768):
    """
    ...
    adapt_lr = 0.01
    meta_lr = 0.001
    model = FrontAndHead(input_dim=input_dim_F, hid_dim=hid_dim_F, num_classes=2,  # 0 or 1
                         task_type="multi_label_classification",
                         token_num=token_num_F, cross_prune=0.1, inner_prune=0.3)

    # load pre-trained GNN
    gnn = GNN(input_dim=input_dim_G, hid_dim=hidden_dim_G, out_dim=out_dim_G, gcn_layer_num=2, gnn_type='TransformerConv')
    # pre_train_path = './pre_trained_gnn/{}.GraphCL.{}.pth'.format(dataname, 'TransformerConv')
    pre_train_path = '/home/f74091093/ProG/pre_trained_gnn/CiteSeer.GraphCL.TransformerConv.pth'
...
if __name__ == '__main__':
    ...
    meta_train_task_id_list = [20, 21, 22, 23,24,25]
    meta_test_task_id_list = [26, 27,28]
    ...
    maml, gnn, opt, lossfn = model_components(input_dim_F,hid_dim_F,token_num_F,input_dim_G,hidden_dim_G,out_dim_G)

    # meta training on source tasks
    meta_train_maml(50, maml, gnn, lossfn, opt, meta_train_task_id_list,
    ...
    # dd/mm/YY
    d1 = today.strftime("%d_%m,%H_%M_%S")

    torch.save(maml.module.PG.state_dict(), f'/home/f74091093/PR_parameter/PG_{input_dim_F}_{hid_dim_F}_{token_num_F}_{input_dim_G}_{hidden_dim_G}_{out_dim_G}_{d1}.pth')