...
def model_components():
    ...
    #                      task_type="multi_label_classification",
    #                      token_num=3, cross_prune=0.1, inner_prune=0.3)
    
    model = FrontAndHead(input_dim=100, hid_dim=100, num_classes=2,  # 0 or 1
                         task_type="multi_label_classification",
                         token_num=3, cross_prune=0.1, inner_prune=0.3)
    
    # self.PG = HeavyPrompt(token_dim=768, token_num=3, cross_prune=0.1,
    #                           inner_prune=0.01)

    # load pre-trained GNN
    # gnn = GNN(input_dim=768, hid_dim=100, out_dim=100, gcn_layer_num=2, gnn_type='TransformerConv')
