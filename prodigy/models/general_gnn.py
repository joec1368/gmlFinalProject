...
import numpy as np
from .ProG.ProG.prompt import HeavyPrompt, LightPrompt,FrontAndHead
from .ProG.ProG.meta import MAML
...
class SingleLayerGeneralGNN(torch.nn.Module):
    def __init__(self, layer_list, initial_label_mlp=torch.nn.Identity(), initial_input_mlp=torch.nn.Identity(),
        ...
        self.PG = HeavyPrompt(token_dim=768, token_num=3, cross_prune=0.1,
                              inner_prune=0.01)
        model = FrontAndHead(input_dim=768, hid_dim=768, num_classes=2,  # 0 or 1
                         task_type="multi_label_classification",
                         token_num=10, cross_prune=0.1, inner_prune=0.3)
        maml = MAML(model, lr=0.01, first_order=False, allow_nograd=True)
        maml.PG.load_state_dict(torch.load("/home/f74091093/PR_parameter/PG.pth"))
        self.PG = maml.PG

        # self.PG = HeavyPrompt(token_dim=768, token_num=3, cross_prune=0.1,
        #                 inner_prune=0.01)

        # self.PG = LightPrompt(100,3,1,0.01)
    ...
    def forward(self, graph, x_label, y_true_matrix, metagraph_edge_index, metagraph_edge_attr, query_set_mask, input_seqs=None, query_seqs=None, query_seqs_gt=None, task_mask=None):
        ...
        adjust = True
        if adjust == True:
            new_graph_x = self.PG(graph)
            graph.x = new_graph_x.x
            new_graph = copy.copy(new_graph_x)
            graph.x_orig = new_graph.x
            graph.edge_index = new_graph_x.edge_index
            graph.batch = new_graph_x.batch
            supernode_idx = graph.supernode + graph.ptr[:-1]
            graph.num_nodes = graph.x.shape[0]
            num_node = graph.x.shape[0]
            num_drop = int(num_node * 0.3)
            node_drop = random.sample(range(num_node), num_drop)
            node_mask = torch.ones(num_node, dtype=bool).to(graph.x.device)
            ...
            node_mask[graph.supernode[0]] = True  # super node
        ...
        for module in self.layer_list:
            ...
            if isinstance(module, MetagraphLayer): # third run, MetaGNN
                if x_input is None:
                ...
                if self.params["skip_path"]:
                    ...
                    x_label = new_x_label
            elif isinstance(module, SupernodeAggrLayer): # second run , BgGraphToSupernodePropagator
                ...
                if self.params["skip_path"]:
                    ...
                    graph.x = new_graph_x
            elif isinstance(module, BackgroundGNNLayer): # first run, MultiLayerGNN
